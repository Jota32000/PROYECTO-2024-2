...
class Cableado:
    ...
    def dibujar_cables(self,screen,cables):
        for cable in cables:
    ...
    def finalizar_cable(self, conector_siguiente,conectores,cables):
        ...
        if ((self.inicio_cable.fase and conector_siguiente.neutro) or (
            ...
            print("----------------------------------")
            Conector.activar_explosion(conector_siguiente)
            self.dibujando_cable = False
            self.inicio_cable = None
            return
        ...
        if not self.quitar_cable(self.inicio_cable, conector_siguiente,conectores,cables):
            for cable in cables:
    ...
    def quitar_cable(self, start,end,conectores,cables):
        for cable in cables:
    ...
    def dibujar_cable_actual(self,screen,conectores):
        ...
        for c in conectores:  # busca las pilas y las envia a cambiar o no estado fase / neutro
            if c.nombre == "pila+":
                if not c.conexiones:
                    c.padre = c
                Conector.energy_protoboard(self,c,conectores)
